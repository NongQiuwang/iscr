  ...
  <div class="admin-page">
    <div class="page-header">
      <div class="header-info">
        <h2>VehicleAdmin</h2>
        <p>Manage all rental vehicle information in the system</p>
      </div>
    ...
    <div class="admin-card">
      ...
      <div class="search-section">
        <el-input v-model="searchForm.brand" placeholder="Brand" class="search-item" clearable @change="fetchData" />
        <el-input v-model="searchForm.series" placeholder="Series" class="search-item" clearable @change="fetchData" />
      ...
      <el-table :data="tableData" v-loading="loading" class="flat-table">
        ...
        <el-table-column label="Vehicle Info" min-width="200">
        ...
        <el-table-column prop="dailyRent" label="Daily Rate" width="120">
          <template #default="{ row }">
        ...
        <el-table-column prop="status" label="Status" width="100">
          <template #default="{ row }">
            <el-tag size="small" :type="row.status === 0 ? 'success' : row.status === 1 ? 'warning' : 'info'" effect="light">
              {{ row.status === 0 ? 'Available' : row.status === 1 ? 'Rented' : 'Maintenance' }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px" class="clean-dialog">
      <el-form :model="form" :rules="rules" ref="formRef" label-position="top">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="Daily Rate" prop="dailyRent">
          ...
          <el-col :span="12">
            <el-form-item label="Status">
              <el-select v-model="form.status" style="width: 100%">
                <el-option label="Available" :value="0" />
                <el-option label="Rented" :value="1" />
                <el-option label="Maintenance" :value="2" />
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        <el-form-item label="Vehicle Image">
          ...
          >
            <div v-if="form.imagePath" class="upload-preview">
              ...
              <div class="upload-mask">Change Image</div>
            </div>
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="handleSubmit" :loading="submitLoading">Save</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import { getAllCars, addCar, updateCar, deleteCar } from '@/api/modules/car'
...
const uploadAction = `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8080/iscr'}/api/upload/car-image`
...
const rules = {
  brand: [{ required: true, message: 'Required', trigger: 'blur' }],
  series: [{ required: true, message: 'Required', trigger: 'blur' }],
  dailyRent: [{ required: true, message: 'Required', trigger: 'blur' }]
}
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('Delete this vehicle?', 'Notice', { type: 'warning', confirmButtonText: 'Delete' })
